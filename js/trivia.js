...
const preguntas = [{
    ...
    pregunta: "¿Qué etiqueta HTML representa un texto en cursiva?", opciones: ["<i>", "<em>", "<italic>"], respuesta: "<i>"
}, {
...
const automatiza = () => {
    ...
    intervalo = setTimeout(() => {
        generaPregunta();
        if (parseInt(puntos.innerText) > 0 && parseInt(puntos.innerText) < 10) {
            puntos.innerText = parseInt(puntos.innerText) - parseInt(puntos.innerText);
            error.play();
        } else if (parseInt(puntos.innerText) !== 0) {
            puntos.innerText = (parseInt(puntos.innerText)) - 10;
            error.play();
        } else {
            clearTimeout(intervalo);
            confirm("Hola Estás ahi ?", "Si")
            generaPregunta();
        }
    }, 16000);
};
...
const comprueba = (target) => {
    if (preguntas[preguntaAleatoria].respuesta === target.innerText) {
        ...
        if (parseInt(puntos.innerText) >= 3) {
            puntos.innerText = (parseInt(puntos.innerText)) - 3;
        }
